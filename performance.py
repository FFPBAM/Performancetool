...
def drawdown_euro_from_index(idx: np.ndarray) -> np.ndarray:
    """Drawdown in absoluten Euro-Beträgen (negativ)."""
    peak = np.maximum.accumulate(idx)
    return idx - peak
...
def calc_max_drawdown(idx_after: np.ndarray, dates_list):
    """Gibt (max_dd_value, max_dd_date) zurück."""
    dd = drawdown_from_index(idx_after)
    min_idx = np.argmin(dd)
    return float(dd[min_idx]), dates_list[min_idx]

def calc_max_drawdown_euro(idx_after: np.ndarray, dates_list):
    """Max Drawdown in Euro (absolut, negativ)."""
    dd_eur = drawdown_euro_from_index(idx_after)
    min_idx = np.argmin(dd_eur)
    return float(dd_eur[min_idx]), dates_list[min_idx]

def calc_calmar_ratio(cagr: float | None, max_dd: float | None) -> float | None:
    """Calmar Ratio = CAGR / |Max Drawdown|"""
    if cagr is None or max_dd is None or max_dd == 0:
        return None
    return cagr / abs(max_dd)

def calc_drawdown_recovery(idx_after: np.ndarray, dates_list):
    """
    Berechnet Recovery-Dauer vom Max-Drawdown-Tief zurück zum vorherigen Peak.
    Gibt zurück: (recovery_days: int | None, recovery_date | None)
    None = noch nicht erholt.
    """
    dd = drawdown_from_index(idx_after)
    min_idx = np.argmin(dd)

    # Suche ab dem Tief den ersten Punkt wo DD wieder >= 0 (also Index >= Peak)
    for i in range(min_idx + 1, len(dd)):
        if dd[i] >= 0.0:
            # Erholung gefunden
            recovery_date = dates_list[i]
            trough_date = dates_list[min_idx]
            if isinstance(recovery_date, pd.Timestamp) and isinstance(trough_date, pd.Timestamp):
                recovery_days = (recovery_date - trough_date).days
            else:
                recovery_days = i - min_idx
            return recovery_days, recovery_date

    return None, None  # noch nicht erholt

def calc_max_drawdown_duration(idx_after: np.ndarray, dates_list):
    """
    Längste zusammenhängende Phase unter dem Peak (in Tagen).
    Gibt (duration_days, start_date, end_date) zurück.
    """
    dd = drawdown_from_index(idx_after)
    peak = np.maximum.accumulate(idx_after)
    ...
    for i in range(len(dd)):
        if dd[i] < 0:
            ...
            if current_start is not None:
                # Phase beendet
                d_start = dates_list[current_start]
                d_end = dates_list[i]
                if isinstance(d_start, pd.Timestamp) and isinstance(d_end, pd.Timestamp):
    ...
    # Falls noch in einer DD-Phase am Ende
    if current_start is not None:
        d_start = dates_list[current_start]
        d_end = dates_list[-1]
        if isinstance(d_start, pd.Timestamp) and isinstance(d_end, pd.Timestamp):
...
def build_bar_chart(bar_df1, label_1, bench_name_1,
    ...
    COLOR_PORT1 = "#B8973A"; COLOR_PORT2 = "#2C5F8A"
    COLOR_BM1 = "#A8CBE8"; COLOR_BM2 = "#7FB5D5"; BG = "#1B3A5C"
    fig = go.Figure()
    col_port1 = f"{label_1} (nach Kosten)"
    if col_port1 in bar_df1.columns:
        vals1 = bar_df1[col_port1].tolist()
        fig.add_trace(go.Bar(name=f"{label_1} (nach Kosten)", x=bar_df1["label"], y=vals1,
            marker_color=COLOR_PORT1, text=[f"{v:+.2f}%" for v in vals1], textposition="outside",
    ...
    if "ret_bm_raw" in bar_df1.columns and bar_df1["ret_bm_raw"].notna().any():
        ...
        fig.add_trace(go.Bar(name=bench_name_1, x=bar_df1["label"], y=bm_vals1,
            marker_color=COLOR_BM1, text=[f"{v:+.2f}%" if pd.notna(v) else "" for v in bm_vals1],
    ...
    if bar_df2 is not None and label_2 is not None:
        ...
        if col_port2 in bar_df2.columns:
            ...
            fig.add_trace(go.Bar(name=f"{label_2} (nach Kosten)", x=bar_df2["label"], y=vals2,
                marker_color=COLOR_PORT2, text=[f"{v:+.2f}%" for v in vals2], textposition="outside",
                textfont=dict(size=11, color="white"), cliponaxis=False))
        if (bench_name_2 and bench_name_2 != bench_name_1
            ...
            fig.add_trace(go.Bar(name=bench_name_2, x=bar_df2["label"], y=bm_vals2,
                marker_color=COLOR_BM2, text=[f"{v:+.2f}%" if pd.notna(v) else "" for v in bm_vals2],
                textposition="outside", textfont=dict(size=11, color="white"), cliponaxis=False))
    ...
    for col in [col_port1] + ([f"{label_2} (nach Kosten)"] if label_2 else []):
        ...
        if col in src.columns: all_vals += src[col].dropna().tolist()
    if "ret_bm_raw" in bar_df1.columns: all_vals += bar_df1["ret_bm_raw"].dropna().tolist()
    if bar_df2 is not None and "ret_bm_raw" in bar_df2.columns: all_vals += bar_df2["ret_bm_raw"].dropna().tolist()
    if all_vals:
        y_min = min(all_vals) * 1.45 if min(all_vals) < 0 else -2
        y_max = max(all_vals) * 1.45 if max(all_vals) > 0 else 2
    else: y_min, y_max = -10, 10
    fig.update_layout(
...
def _mpl_line_chart(x_dates, traces, y_label, title, use_volume):
    ...
    fig.patch.set_facecolor(FFPB_DARK); ax.set_facecolor(FFPB_DARK)
    ...
    for spine in ax.spines.values(): spine.set_color("#3A5A7C")
    ...
    if port_vals:
        ...
        for bar, v in zip(bars1, port_vals):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height(),
                    f"{v:+.2f}%", ha="center", va="bottom" if v >= 0 else "top", fontsize=7, color="white")
    if has_bm:
        bm_clean = [v if pd.notna(v) else 0 for v in bm_vals]
        bars2 = ax.bar(x + width / 2, bm_clean, width, label=bench_name, color=FFPB_LIGHT)
...
def generate_pdf(
    ...
    bench_name_1, bench_name_2, bench_text_1, bench_text_2,
    fee_pct_1, fee_pct_2, anlagevolumen, use_volume,
    start_date, end_date, x_dates, line_traces, y_label,
    show_drawdown, dd_traces, show_table, df_roll,
    show_bar, bar_data_list, metrics_data,
):
    buf = io.BytesIO()
    doc = SimpleDocTemplate(buf, pagesize=A4,
        topMargin=15*mm, bottomMargin=15*mm, leftMargin=15*mm, rightMargin=15*mm)
    styles = getSampleStyleSheet()
    style_title = ParagraphStyle("FFPBTitle", parent=styles["Title"],
        textColor=HexColor(FFPB_DARK), fontSize=16, spaceAfter=6)
    style_subtitle = ParagraphStyle("FFPBSub", parent=styles["Heading2"],
        textColor=HexColor(FFPB_DARK), fontSize=12, spaceAfter=4, spaceBefore=10)
    style_normal = ParagraphStyle("FFPBNormal", parent=styles["Normal"],
        textColor=HexColor("#333333"), fontSize=9, leading=12)
    style_small = ParagraphStyle("FFPBSmall", parent=styles["Normal"],
    ...
    if logo_path and os.path.exists(logo_path):
        logo_w = 50 * mm; logo_h = logo_w * logo_aspect
    ...
    if label_2: meta_lines.append(f"<b>Vergleichsportfolio:</b> {label_2}")
    ...
    if use_volume: meta_lines.append(f"<b>Anlagevolumen:</b> {fmt_eur_de(anlagevolumen)}")
    for line in meta_lines: story.append(Paragraph(line, style_normal))
    story.append(Spacer(1, 4 * mm))

    # Kennzahlen
    story.append(Paragraph("Kennzahlen (nach Kosten)", style_subtitle))
    for m in metrics_data:
        parts = [f"<b>{m['label']}:</b>"]
        if m.get("auflagedatum"): parts.append(f"Auflagedatum im PM: {fmt_date_de(m['auflagedatum'])}")
        if m.get("cagr") is not None: parts.append(f"CAGR: {fmt_pct_de(m['cagr'])}")
        if m.get("vola") is not None: parts.append(f"Vola p.a.: {fmt_pct_de(m['vola'])}")
        if m.get("calmar") is not None: parts.append(f"Calmar: {m['calmar']:.2f}".replace(".", ","))
        if use_volume and m.get("endwert") is not None: parts.append(f"Endwert: {fmt_eur_de(m['endwert'])}")
        if m.get("max_dd_val") is not None:
            dd_str = f"Max. DD: {fmt_pct_de(m['max_dd_val'])} am {fmt_date_de(m['max_dd_date'])}"
            if use_volume and m.get("max_dd_eur") is not None:
                dd_str += f" ({fmt_eur_de(m['max_dd_eur'])})"
            parts.append(dd_str)
        if m.get("recovery_days") is not None:
            parts.append(f"Recovery: {m['recovery_days']} Tage")
        elif m.get("max_dd_val") is not None:
            parts.append("Recovery: noch nicht erholt")
        if m.get("max_dd_dur") is not None and m["max_dd_dur"] > 0:
            parts.append(f"Längste DD-Phase: {m['max_dd_dur']} Tage")
        story.append(Paragraph(" | ".join(parts), style_normal))
    story.append(Spacer(1, 5 * mm))

    # Line Chart
    story.append(Paragraph("Performance-Index", style_subtitle))
    line_title = f"Wertentwicklung in Euro ({fmt_eur_de(anlagevolumen)})" if use_volume else "Performance-Index (Start = 100)"
    ...
    if show_drawdown and dd_traces:
        story.append(Spacer(1, 4*mm))
        dd_title = "Drawdown in Euro (nach Kosten)" if use_volume else "Drawdown (nach Kosten)"
        story.append(Paragraph(dd_title, style_subtitle))
        dd_buf = _mpl_drawdown_chart(x_dates, dd_traces, dd_title, use_volume)
    ...
    if show_table and df_roll is not None and not df_roll.empty:
        ...
        if logo_path and os.path.exists(logo_path):
            lw = 35*mm; story.append(RLImage(logo_path, width=lw, height=lw*logo_aspect))
            story.append(Spacer(1, 3*mm))
        ...
        col_count = len(header); first_col_w = 45*mm
        other_col_w = (170*mm - first_col_w) / max(col_count - 1, 1)
    ...
    story.append(Paragraph(f"Erstellt am {fmt_date_de(dt.date.today())} | Fürst Fugger Privatbank", style_small))
...
def build_portfolio_timeseries(files, mapping):
    ...
    for path in files:
        ...
        try: fee_default = float(mapping.loc[mapping["Inhaber"] == portfolio, "Honorarsatz Standard"].values[0])
        except Exception: fee_default = 0.0
        idx = dates.iloc[1:].reset_index(drop=True)
        df = pd.DataFrame(index=idx); df.index.name = "Datum"
        df["ret_port"] = ret_port
        df["ret_bm"] = ret_bm if (ret_bm is not None and len(ret_bm) == len(df)) else np.nan
        df["fee_default"] = fee_default
        df = df.sort_index(); df.attrs["benchmark_name"] = bench_name
        out[portfolio] = df
    return out
...
with st.sidebar:
    ...
    if len(files) == 0:
        st.error(f"Keine Dateien gefunden für Tag {date_tag}."); st.stop()
    data = build_portfolio_timeseries(files, mapping)

    col_display = name_mapping.columns[0]; col_csv_key = name_mapping.columns[1]; col_bench = name_mapping.columns[3]
    available_csv_names = set(data.keys())
    name_mapping_filtered = name_mapping[name_mapping[col_csv_key].isin(available_csv_names)].copy()
    ...
    if len(display_names_ordered) == 0:
        st.error("Keine Portfolios zugeordnet."); st.stop()
    ...
    portfolio_sel2 = display_sel_2 = None
    if show_compare:
    ...
    st.markdown("---"); st.subheader("Anlagevolumen")
    anlagevolumen = st.number_input("Anlagevolumen in € (optional)",
        ...
        help="Wenn > 0: Chart zeigt Wertentwicklung in Euro.")
    ...
    fee_pct_1 = st.number_input(f"Kosten p.a. (%) – {display_sel_1}",
        min_value=0.0, max_value=20.0, value=float(round(fee_default_dec_1 * 100, 4)), step=0.05)
    fee_dec_1 = fee_pct_1 / 100.0
    fee_dec_2 = fee_pct_2 = None
    if show_compare and portfolio_sel2:
        ...
        fee_pct_2 = st.number_input(f"Kosten p.a. (%) – {display_sel_2}",
            min_value=0.0, max_value=20.0, value=float(round(fee_default_dec_2 * 100, 4)), step=0.05)
...
raw_min_1 = data[portfolio_sel].index.min().date(); raw_max_1 = data[portfolio_sel].index.max().date()
if show_compare and portfolio_sel2:
    raw_min_2 = data[portfolio_sel2].index.min().date(); raw_max_2 = data[portfolio_sel2].index.max().date()
    min_d = max(raw_min_1, raw_min_2); max_d = min(raw_max_1, raw_max_2)
else:
    min_d = raw_min_1; max_d = raw_max_1
...
if start_date > end_date:
    st.error("Startdatum darf nicht nach dem Enddatum liegen."); st.stop()
...
if show_compare and portfolio_sel2:
    ...
    joined = (df1[["ret_port", "ret_bm"]].rename(columns={"ret_port": "ret_port_1", "ret_bm": "ret_bm_1"})
        .join(df2_raw[["ret_port", "ret_bm"]].rename(columns={"ret_port": "ret_port_2", "ret_bm": "ret_bm_2"}), how="inner"))
    if joined.empty:
        st.error("Kein gemeinsamer Datumsbereich."); st.stop()
...
# Drawdown immer auf Basis 100 für Prozent
...
max_dd_eur_1 = None
if use_volume:
    max_dd_eur_1, _ = calc_max_drawdown_euro(idx_after_1, x_dates)
calmar_1 = calc_calmar_ratio(cagr_1, max_dd_val_1)
recovery_days_1, recovery_date_1 = calc_drawdown_recovery(idx_af_1_100, x_dates)
max_dd_dur_1, dd_dur_start_1, dd_dur_end_1 = calc_max_drawdown_duration(idx_af_1_100, x_dates)

cagr_2 = vola_2 = endwert_2 = max_dd_val_2 = max_dd_date_2 = max_dd_eur_2 = None
calmar_2 = recovery_days_2 = recovery_date_2 = max_dd_dur_2 = dd_dur_start_2 = dd_dur_end_2 = None
if df2 is not None:
    n_days_2 = len(ret2)
    ...
    if use_volume:
        max_dd_eur_2, _ = calc_max_drawdown_euro(idx_after_2, x_dates)
    calmar_2 = calc_calmar_ratio(cagr_2, max_dd_val_2)
    recovery_days_2, recovery_date_2 = calc_drawdown_recovery(idx_af_2_100, x_dates)
    max_dd_dur_2, dd_dur_start_2, dd_dur_end_2 = calc_max_drawdown_duration(idx_af_2_100, x_dates)

metrics_data = [{
    "label": label_1, "auflagedatum": auflagedatum_1,
    "cagr": cagr_1, "vola": vola_1, "endwert": endwert_1, "calmar": calmar_1,
    "max_dd_val": max_dd_val_1 if show_drawdown else None,
    "max_dd_date": max_dd_date_1 if show_drawdown else None,
    "max_dd_eur": max_dd_eur_1 if show_drawdown else None,
    "recovery_days": recovery_days_1 if show_drawdown else None,
    "recovery_date": recovery_date_1 if show_drawdown else None,
    "max_dd_dur": max_dd_dur_1 if show_drawdown else None,
}]
if df2 is not None and label_2:
    metrics_data.append({
        "label": label_2, "auflagedatum": auflagedatum_2,
        "cagr": cagr_2, "vola": vola_2, "endwert": endwert_2, "calmar": calmar_2,
        "max_dd_val": max_dd_val_2 if show_drawdown else None,
        "max_dd_date": max_dd_date_2 if show_drawdown else None,
        "max_dd_eur": max_dd_eur_2 if show_drawdown else None,
        "recovery_days": recovery_days_2 if show_drawdown else None,
        "recovery_date": recovery_date_2 if show_drawdown else None,
        "max_dd_dur": max_dd_dur_2 if show_drawdown else None,
    })

# ── Kennzahlen über dem Chart ─────────────────────────────────────────────
st.subheader("📊 Kennzahlen (nach Kosten)")
display_metrics(label_1, cagr_1, vola_1, endwert_1, use_volume, auflagedatum_1, calmar_1)
if df2 is not None and label_2:
    display_metrics(label_2, cagr_2, vola_2, endwert_2, use_volume, auflagedatum_2, calmar_2)
...
fig = go.Figure()
fig.add_trace(go.Scatter(x=x_dates, y=idx_after_1, mode="lines",
    name=f"{label_1} – nach Kosten ({fee_pct_1:.2f}%)"))
if idx_after_2 is not None:
    fig.add_trace(go.Scatter(x=x_dates, y=idx_after_2, mode="lines",
        name=f"{label_2} – nach Kosten ({(fee_pct_2 or 0.0):.2f}%)"))
if show_vorkosten:
    fig.add_trace(go.Scatter(x=x_dates, y=idx_before_1, mode="lines", name=f"{label_1} – vor Kosten"))
    if idx_before_2 is not None:
        fig.add_trace(go.Scatter(x=x_dates, y=idx_before_2, mode="lines", name=f"{label_2} – vor Kosten"))
if show_benchmark and df1["ret_bm"].notna().any():
    ...
    fig.add_trace(go.Scatter(x=x_dates, y=idx_bm_1, mode="lines", name=f"Benchmark {label_1}: {bench_name_1}"))
    if df2 is not None and df2["ret_bm"].notna().any():
        ...
        fig.add_trace(go.Scatter(x=x_dates, y=idx_bm_2, mode="lines", name=f"Benchmark {label_2}: {bench_name_2}"))

fig.update_layout(height=550, xaxis_title="Datum", xaxis=dict(tickformat="%d.%m.%Y"),
    yaxis_title=y_label, yaxis=dict(tickformat=",.2f" if use_volume else None),
    legend_title_text="Reihen", hovermode="x unified")
...
if show_drawdown:
    st.markdown("---")
    display_drawdown_metrics(label_1, max_dd_val_1, max_dd_date_1, max_dd_eur_1, use_volume,
                             recovery_days_1, recovery_date_1, max_dd_dur_1, dd_dur_start_1, dd_dur_end_1)
    if df2 is not None and label_2:
        display_drawdown_metrics(label_2, max_dd_val_2, max_dd_date_2, max_dd_eur_2, use_volume,
                                 recovery_days_2, recovery_date_2, max_dd_dur_2, dd_dur_start_2, dd_dur_end_2)

    fig_dd = go.Figure()
    if use_volume:
        # Drawdown in Euro
        dd1_vals = drawdown_euro_from_index(idx_after_1)
        fig_dd.add_trace(go.Scatter(x=x_dates, y=dd1_vals, mode="lines",
            name=f"{label_1} – Drawdown in € (nach Kosten)"))
        if df2 is not None:
            dd2_vals = drawdown_euro_from_index(idx_after_2)
            fig_dd.add_trace(go.Scatter(x=x_dates, y=dd2_vals, mode="lines",
                name=f"{label_2} – Drawdown in € (nach Kosten)"))
        fig_dd.update_layout(height=350, xaxis_title="Datum", xaxis=dict(tickformat="%d.%m.%Y"),
            yaxis_title="Drawdown in €", yaxis=dict(tickformat=",.0f"), hovermode="x unified")
    else:
        # Drawdown in Prozent
        dd1_vals = drawdown_from_index(idx_af_1_100)
        fig_dd.add_trace(go.Scatter(x=x_dates, y=dd1_vals, mode="lines",
            name=f"{label_1} – Drawdown (nach Kosten)"))
        if df2 is not None:
            dd2_vals = drawdown_from_index(idx_af_2_100)
            fig_dd.add_trace(go.Scatter(x=x_dates, y=dd2_vals, mode="lines",
...
if show_table:
    ...
    df_roll = build_rolling_table(idx_before_1=s_before_1_tbl, idx_after_1=s_after_1_tbl, label_1=label_1,
        idx_before_2=s_before_2_tbl, idx_after_2=s_after_2_tbl, label_2=label_2, since_label=since_label)
...
if show_bar:
    st.markdown("---"); st.subheader("📊 Performance im Benchmarkvergleich (blockweise)")
    bar_left, bar_right = st.columns([1, 3])
    with bar_left:
        bar_mode = st.radio("Zeitraum-Einteilung", options=["Kalenderjahre", "Quartale", "Benutzerdefiniert"], index=0)
        custom_start_bar = custom_end_bar = None
        if bar_mode == "Benutzerdefiniert":
            custom_start_bar = st.date_input("Von", value=start_date, min_value=min_d, max_value=max_d,
                format="DD.MM.YYYY", key="bar_von")
            custom_end_bar = st.date_input("Bis", value=end_date, min_value=min_d, max_value=max_d,
    ...
    def _render_bar(df_src, fee, label, bench_name, bench_text, container):
        bar_df = compute_bar_data(df_src, fee_dec=fee, mode=bar_mode, label=label,
            custom_start=custom_start_bar, custom_end=custom_end_bar)
        if bar_df.empty: container.info(f"Keine Daten für {label}."); return
    ...
    with bar_right:
        ...
        if show_benchmark: show_benchmark_composition(label_1, bench_text_1)
        if df2 is not None and fee_dec_2 is not None and portfolio_sel2:
            ...
            if show_benchmark: show_benchmark_composition(label_2, bench_text_2)

# ── PDF ──────────────────────────────────────────────────────────────────
st.markdown("---")
pdf_line_traces = [(f"{label_1} – nach Kosten ({fee_pct_1:.2f}%)", idx_after_1)]
if idx_after_2 is not None:
...
if show_benchmark and df1["ret_bm"].notna().any():
    ...
    pdf_line_traces.append((f"Benchmark {label_1}: {bench_name_1}", make_index_from_returns(ret_bm_1_pdf, startwert=startwert)))
    if df2 is not None and df2["ret_bm"].notna().any():
        ret_bm_2_pdf = df2["ret_bm"].fillna(0.0).to_numpy(dtype=float)
        pdf_line_traces.append((f"Benchmark {label_2}: {bench_name_2}", make_index_from_returns(ret_bm_2_pdf, startwert=startwert)))

pdf_dd_traces = []
if show_drawdown:
    if use_volume:
        pdf_dd_traces.append((f"{label_1} – DD in €", drawdown_euro_from_index(idx_after_1)))
        if df2 is not None:
            pdf_dd_traces.append((f"{label_2} – DD in €", drawdown_euro_from_index(idx_after_2)))
    else:
...
if st.button("📄 PDF Report erstellen"):
    with st.spinner("PDF wird erstellt..."):
        pdf_bytes = generate_pdf(
            logo_path=logo_path, label_1=label_1, label_2=label_2,
            ...
            start_date=start_date, end_date=end_date, x_dates=x_dates,
    ...
    st.download_button(label="⬇️ PDF herunterladen", data=pdf_bytes,
        file_name=f"Performance_{label_1}_{fmt_date_de(start_date)}-{fmt_date_de(end_date)}.pdf",
        mime="application/pdf")
    st.success("PDF erfolgreich erstellt!")

with st.expander("Details / Debug"):
    st.write("Dateien:", len(files), "| Auto-Tag:", auto_tag)
    st.write("Zeitraum:", fmt_date_de(start_date), "–", fmt_date_de(end_date))
    st.write(f"Auflagedatum {label_1}:", fmt_date_de(auflagedatum_1))
    if auflagedatum_2: st.write(f"Auflagedatum {label_2}:", fmt_date_de(auflagedatum_2))
    st.write(f"CAGR 1: {fmt_pct_de(cagr_1) if cagr_1 else '–'} | Vola 1: {fmt_pct_de(vola_1) if vola_1 else '–'}")
    st.write(f"Calmar 1: {calmar_1:.2f}" if calmar_1 else "Calmar 1: –")
    st.write(f"Max DD 1: {fmt_pct_de(max_dd_val_1)} am {fmt_date_de(max_dd_date_1)}")
    st.write(f"Recovery 1: {recovery_days_1} Tage" if recovery_days_1 else "Recovery 1: noch nicht erholt")
    st.write(f"Längste DD-Phase 1: {max_dd_dur_1} Tage")
    st.write("Rows P1:", len(df1))
    if df2 is not None: st.write("Rows P2:", len(df2))
